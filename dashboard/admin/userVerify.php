...
if (isset($_POST["verify"])) {
    ...
    $userId = $_POST["user_id"];
    $getVerificationStatusQuery = "select user_verification_try from user where user_id=$userId";
    $getVerificationStatusResult = mysqli_query($connection, $getVerificationStatusQuery);
    $row = mysqli_fetch_assoc($getVerificationStatusResult);
    $verificationStatus = $row["user_verification_try"];
    $verificationStatusUpdate = $verificationStatus + 1;
    $verifyQuery = "UPDATE user SET user_status = 2, user_verification_try='$verificationStatusUpdate' WHERE user_id = '$userId'";
